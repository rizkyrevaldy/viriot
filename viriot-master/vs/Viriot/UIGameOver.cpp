...
UIGameOver::UIGameOver(int screenWidth, int screenHeight)
{
...
void UIGameOver::RenderText(string text, GLfloat x, GLfloat y, GLfloat scale, vec3 color, bool centeredPivot)
{
...
void UIGameOver::InitButton(string buttons[], string hover_buttons[]) {
...
void UIGameOver::RenderButton(float posX[], float posY[]) {
	// Enable transparency
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	//int n = 5;
...
GLuint UIGameOver::BuildShader(const char* vertexPath, const char* fragmentPath, const char* geometryPath)
{
...
void UIGameOver::UseShader(GLuint program)
{
	// Uses the current shader
	glUseProgram(program);
}

void UIGameOver::Err(string errorString)
{
...
void UIGameOver::CheckShaderErrors(GLuint shader, string type)
{
	GLint success;
	GLchar infoLog[1024];
	if (type != "PROGRAM")
