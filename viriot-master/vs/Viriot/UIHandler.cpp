...
UIHandler::UIHandler(int screenWidth, int screenHeight)
{
...
void UIHandler::RenderText(string text, GLfloat x, GLfloat y, GLfloat scale, vec3 color, bool centeredPivot)
{
...
void UIHandler::InitButton(string buttons[], string hover_buttons[]) {
...
void UIHandler::RenderButton(float posX[], float posY[]) {
	// Enable transparency
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	int n = 5;
...
GLuint UIHandler::BuildShader(const char* vertexPath, const char* fragmentPath, const char* geometryPath)
{
...
void UIHandler::UseShader(GLuint program)
{
	// Uses the current shader
	glUseProgram(program);
}

void UIHandler::Err(string errorString)
{
...
void UIHandler::CheckShaderErrors(GLuint shader, string type)
{
	GLint success;
	GLchar infoLog[1024];
	if (type != "PROGRAM")
